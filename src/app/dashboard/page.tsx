...
export default function DashboardPage() {
  ...
  const { tweets, loading, error, refreshFeed, loadMoreTweets, hasMore } =
    useFeed();
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-background text-foreground relative overflow-hidden transition-all duration-300">
      ...
      <div className="xl:ml-64 flex-1 relative z-10 w-full xl:w-auto pt-14 sm:pt-16 xl:pt-0 pb-14 sm:pb-16 xl:pb-0">
        {/* Stories section - Exactly like the image */}
        <div className="px-6 py-6">
          <h2 className="text-xl font-bold text-white mb-6">Stories</h2>
          <Story onStoryClick={handleOpenStory} />
        </div>
                  ...
                  {tweets.length === 0 && !loading ? (
                    <div className="text-center py-8 sm:py-12 lg:py-16">
                      ...
                      <p className="text-muted-foreground text-base sm:text-lg px-4">
                        Commencez à suivre des personnes pour voir leurs tweets
                        ici !
                      </p>
                    </div>
                  ) : (
